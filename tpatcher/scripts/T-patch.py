...
def get_callbacks(args_):
    ...
    return [lr_callback, early_stopping_callback]
...
def editing(e_t, a_t):
    ...
    if args.task == 'fever':
        ...
        need_edit, ber, rephrase_num = edit_or_not_seq2seq(editor.editor, editor.editor.model, data_point=d0, device=args.device, args=args, before=True)
        if need_edit != 0:
            if args.use_init_weight == 1:
                editor.editor.insert_hidden_detector()
    ...
    if need_edit != 0 and error_count > 0:
        ...
        # )
        # edit_trainer = Trainer(
        #     callbacks=callbacks, logger=TensorBoardLogger(log_dir, name=None),
        #     check_val_every_n_epoch=args.check_val_every_n_epoch, log_every_n_steps=args.check_val_every_n_epoch,
        ...
        edit_trainer = Trainer(
            callbacks=callbacks, logger=TensorBoardLogger(log_dir, name=None),
            check_val_every_n_epoch=args.check_val_every_n_epoch, log_every_n_steps=args.check_val_every_n_epoch,
            max_epochs=args.max_edit_step, num_sanity_val_steps=0, enable_checkpointing=False,
            gradient_clip_val=5.0
        )
        ...
        # for name, param in editor.editor.model.model.transformer.named_parameters():
        #     device_distribution[name] = param.device

        # editor.editor.set_device_distribution(device_distribution)
...
if __name__ == "__main__":
    from src.models.seq2seq_modules import BartSeq2SeqEditor,T5Seq2SeqEditor,T5Seq2Seq
    # from src.models.llama_module import LlaMASeq2Seq, LlaMASeq2SeqEditor
    from src.models.gpt_module import GPTJSeq2Seq, GPTJSeq2SeqEditor
    ...
    TEST_DICT = {'zsre': my_test_seq2seq, 'fever': my_test_binary, 'counterfact': my_test_seq2seq}
    ...
    with open(os.path.join(args.data_path, f"seq_edit_data_{args.task}_{args.model_type}_{args.edit_folder_num}.pkl"), 'rb') as file:
    ...
    if os.path.exists(os.path.join(args.ckpt_path, 'last_model.ckpt')) \
        ...
        if args.task == 'fever':
            ...
            args, _ = parser.parse_known_args()
            editor = T5Seq2SeqEditor(**vars(args))
        elif args.model_type == 'llama':
            parser = LlaMASeq2Seq.add_model_specific_args(parser)
            args, _ = parser.parse_known_args()
            editor = LlaMASeq2SeqEditor(**vars(args))
        elif args.model_type == 'gptj':
            parser = GPTJSeq2Seq.add_model_specific_args(parser)
            args, _ = parser.parse_known_args()
            editor = GPTJSeq2SeqEditor(**vars(args))
        else:
    ...
    # edited_data1 = pickle.loads(open(
    #     '/root/peng/Transformer-Patcher/log/T-patch/zsre/zsre_T-patch_1folders_t5-3b/his_edit_data_2.pkl',
    #     'rb').read())
    # # edited_data2 = pickle.loads(open(
    # #     '/root/peng/Transformer-Patcher/log/T-patch/counterfact/counterfact_T-patch_1folders_gpt-j-6B/his_edit_data_2.pkl',
    # #     'rb').read())
    # edited_data2 = []
    # edited_data = edited_data2 + edited_data1
    # assert type(edited_data1) is list
    # edited_sets = [edited_data_['raw'][0]['src'] for edited_data_ in edited_data]
    # keep_indexes = []
    # for index_, edit_set in tqdm(enumerate(edit_sets), total=len(edit_sets), desc='Filter edited samples'):
    #     edit_dl = DataLoader(dataset=edit_set, batch_size=1, collate_fn=seq_edit_data.edit_data.collate_fn)
    #     d0 = [j for j in edit_dl][0]
    #     if d0['raw'][0]['src'] not in edited_sets:
    #         keep_indexes.append(index_)
    # assert len(keep_indexes) + len(edited_data) == len(edit_sets)
    # edit_sets = [edit_sets[keep_index] for keep_index in keep_indexes]


    s, ds = 0, None
    from tqdm import tqdm
    for s, ds in tqdm(enumerate(edit_sets), total=len(edit_sets), desc='Editing Processing!!'):
        ...
        if edit_times_tmp == edit_times + 1:
            ...
            if args.task == 'fever':
                ...
                edit_is_not_suc, aer, re_num, loc_score, portability_score, unrelated_relation_score, distracting_score, inverse_score, tongyici_score = edit_or_not_seq2seq(editor.editor, editor.editor.model, data_point=d0, device=args.device, args=args)
            LOG.info(f"After editing, {1 - edit_is_not_suc} and {aer} edit example and its rephrases, loc:{loc_score}   portability:{portability_score} \n"
                     f"unrelated relation:{unrelated_relation_score}   distracting:{distracting_score}  inverse:{inverse_score} tongyici:{tongyici_score}")
            ...
            res_out.loc[f].append((loc_score))
            res_out.portability[f].append((portability_score))
            res_out.unrelated_relation[f].append((unrelated_relation_score))
            res_out.distracting[f].append((distracting_score))
            res_out.inverse[f].append((inverse_score))
            res_out.tongyici[f].append((tongyici_score))
            ...
            with open((os.path.join(log_dir, 'edit_schedule.txt')), 'w') as edit_schedule_file:
            ...
            if cur_index >= max_iterations:
                ...
                def dict_to(d, device):
                    ...
                    return new_dict
                print_interval=2
                labels = open('./data/piqa_valid-labels.lst', 'r', encoding='utf-8').readlines()
                labels = [int(label.strip()) for label in labels]

                preds = []
                piqa_data = []
                with open('./data/piqa_valid.jsonl', 'r', encoding='utf-8') as fin:
                    for line in fin.readlines():
                        line_data = json.loads(line.strip())
                        piqa_data.append(line_data)
                assert len(piqa_data) == len(labels)
                tokenizer = GPT2TokenizerFast.from_pretrained('/root/peng/hugging_cache/gpt-j-6B')
                for i, mc_data in tqdm(enumerate(piqa_data), total=len(piqa_data), desc='Test Processing'):

                    if i % print_interval == 0 and i != 0:
                        print(f'Multiple Choice acc is {np.mean(np.equal(labels[:len(preds)], preds))}')

                    goal = mc_data['goal']
                    candidate1 = mc_data['sol1']
                    candidate2 = mc_data['sol2']
                    input1 = f'{goal} {candidate1}'
                    input2 = f'{goal} {candidate2}'
                    inputs_1 = dict_to(tokenizer(input1, truncation=True, max_length=64, return_tensors='pt'),
                                       args.device)
                    inputs_2 = dict_to(tokenizer(input2, truncation=True, max_length=64, return_tensors='pt'),
                                       args.device)
                    loss1 = editor.editor(inputs_1['input_ids'], inputs_1['attention_mask'], 0, return_loss=True)
                    loss2 = editor.editor(inputs_2['input_ids'], inputs_2['attention_mask'], 0, return_loss=True)
                    ...
                    if loss1 == loss2:
                        ...
                        preds.append(1)
                print('***************Common Done***************')
                print(np.mean(np.equal(labels[:len(preds)], preds)))
                exit()
            # if cur_index % log_interval == 0 and cur_index != 0:
            #     editor.editor.clear_editors()
            # cur_index+=1
            # if cur_index >= max_index:
            #     print('***************Sequential Edit Done***************')
            #     break
        end_time = time.time()
        LOG.info(f"Edit Number {s} Time Cost: {end_time-start_time} s ")
    ...
    if args.get_heat_map == 0:
        os.remove(os.path.join(log_dir, "last_model.ckpt"))
