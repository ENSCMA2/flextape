...
class SeqEditDataSet(object):
    ...
    def __init__(self, task_name='fever', tokenizer=None, data_path=None,
                 ...
                 batch_size=128, num_workers=1, loss_as_val_metric=True, example_repeat=1):
        ...
        if task_name == 'fever':
            ...
            self.edit_data = Seq2SeqData(tokenizer=tokenizer, data_path=edit_path, edit=True, validation=True, example_repeat=1)
            self.dev_data = Seq2SeqData(tokenizer=tokenizer, data_path=dev_path, validation=True) #没用
            ...
            self.val_data = Seq2SeqData(tokenizer=tokenizer, data_path=val_path, validation=False) #没用
        ...
        # )
        self.train_sub, self.memory_set = self.train_data_as_val, self.train_data_as_memo
...
class SeqEditResOutput(object):
    ...
    def __init__(self, edit_folder_num=20, save_dir='./'):
        ...
        self.loc = {f: [] for f in range(edit_folder_num)}
        self.portability = {f: [] for f in range(edit_folder_num)}
        self.unrelated_relation = {f: [] for f in range(edit_folder_num)}
        self.distracting = {f: [] for f in range(edit_folder_num)}
        self.inverse = {f: [] for f in range(edit_folder_num)}
        self.tongyici = {f: [] for f in range(edit_folder_num)}
