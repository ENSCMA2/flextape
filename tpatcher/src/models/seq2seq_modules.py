...
class T5Editor(nn.Module):

    def __init__(self,
                 model, hidden_size=1024, device=None,
                 max_add_neuron_num=1,
                 activate_loss='non_use', memory_loss='non_use',
                 freeze_model=True, freeze_k=False, freeze_a=False,
                 memory_size=50000,
        ...
        #     }
        # elif(len(args.gpus) == 5):
        #     device_map = {
        #         0: [_ for _ in range(0, 5)],
        #         1: [_ for _ in range(5, 10)],
        #         2: [_ for _ in range(10, 15)],
        #         3: [_ for _ in range(15, 20)],
        #         4: [_ for _ in range(20, 24)]
        #     }
        # elif(len(args.gpus) == 6):
        #     device_map = {
        ...
        self.detected_modules={'model.decoder.block.23.layer.2.DenseReluDense': 'wi'}

        self.name_edit_type = {
            'model.decoder.block.23.layer.2.DenseReluDense.wi': 'output',
            'model.decoder.block.23.layer.2.DenseReluDense.wo': 'input'
        }
    ...
    def insert_hidden_detector(self):
        self.get_detectors(
            detected_modules={'model.decoder.block.23.layer.2.DenseReluDense': 'wi'},
            memory_loc='bart_seq', hidden_loc='bart_seq'
        )
        self.set_detectors()
    ...
    def forward(self, input_ids, attention_mask, decoder_input_ids, decoder_attention_mask):
        ...
        self.model.model.to(self.device) # For T5
        logits = self.model(
            input_ids, attention_mask,
            decoder_input_ids[:, :-1], decoder_attention_mask[:, :-1]
        )
...
class BartSeq2Seq(LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            ...
            default='data/zsre_data/zsre-dev-kilt.jsonl'
        )
        # par
...
class T5Seq2Seq(LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        ...
        # parser.add_argument("--warmup_updates", type=int, default=500)
        # parser.add_argument("--num_workers", type=int, default=0)
        # parser.add_argument("--model_name", type=str, default="t5-3b-ssm-nq")
        # parser.add_argument("--eps", type=float, default=0.1)
        return parser
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.model.to(self.hparams.device)  # For T5
        except:
            print("The cache can not be used")
            self.tokenizer = T5Tokenizer.from_pretrained(self.hparams.model_name, cache_dir=self.hparams.cache_dir)  # have internet
            self.model = T5ForConditionalGeneration.from_pretrained(self.hparams.model_name, cache_dir=self.hparams.cache_dir)  # have internet
        # self.train_acc = Accuracy()
        # self.valid_acc = Accuracy()
        self.train_acc = Accuracy(task='binary')
        self.valid_acc = Accuracy(task='binary')
...
class T5Seq2SeqEditor(LightningModule):
