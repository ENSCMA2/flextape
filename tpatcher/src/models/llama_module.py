...
from transformers import (
    LlamaTokenizer, LlamaForCausalLM, get_linear_schedule_with_warmup, LlamaConfig
)
...
class LlaMASeq2Seq(LightningModule):
    @staticmethod
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.tokenizer = LlamaTokenizer.from_pretrained(self.hparams.model_name, cache_dir=self.hparams.cache_dir)  # have internet
            self.model = LlamaForCausalLM.from_pretrained(self.hparams.model_name, cache_dir=self.hparams.cache_dir)  # have internet
    ...
    def training_step(self, batch, batch_idx=None):
        ...
        attention_mask = batch["src_trg_attention_mask"]
        output = self.forward(input_ids, attention_mask, input_ids)
        loss, logits = output.loss, output.logits

        # loss, nll_loss = label_smoothed_nll_loss(
        #     logits.log_softmax(-1), batch["src_trg_input_ids"],
        #     epsilon=self.hparams.eps, ignore_index=self.tokenizer.pad_token_id,
        # )

        # ntokens = batch["src_trg_attention_mask"][:, :].sum()
        # loss, nll_loss = loss / ntokens, nll_loss / ntokens
        self.log("loss", loss, on_step=True, on_epoch=False, prog_bar=True, rank_zero_only=True)
        return {"loss": loss}
...
class LlaMAEditor(nn.Module):
    ...
    def __init__(self,
        ...
        #     device_map = {
        #         0: [_ for _ in range(0, 8)],
        #         1: [_ for _ in range(8, 16)],
        #         2: [_ for _ in range(16, 24)],
        #         3: [_ for _ in range(24, 32)]
        #     }
        # else:
        ...
        self.detected_modules=[
            ('model.model.layers.31.mlp', 'gate_proj'),
        ]

        self.name_edit_type = {
            'model.model.layers.31.mlp.gate_proj': 'output',
            'model.model.layers.31.mlp.up_proj': 'output',
            'model.model.layers.31.mlp.down_proj': 'input'
        }
    ...
    def forward(self, input_ids, attention_mask):
        ...
        #     raise ValueError('Device is not enough!')
        # self.model.model.parallelize(device_map=device_map)
        model_named_modules = {x[0]:x[1] for x in self.model.model.model.named_modules()}
        ...
        loss, logits = output.loss, output.logits
        # loss, nll_loss = label_smoothed_nll_loss(
        #     lprobs=logits.log_softmax(-1), target=input_ids,
...
class LlaMASeq2SeqEditor(LightningModule):
    ...
    def reset(self, clear_memory):
        self.editor.reset_model(
            LlaMASeq2Seq.load_from_checkpoint(self.hparams.model_path),
    ...
    def joint_training(self, batch, batch_idx=None):
        input_ids = batch["src_trg_input_ids"]
        res = self.editor(
            input_ids, batch["src_trg_attention_mask"]
        )
        loss = res['loss']
