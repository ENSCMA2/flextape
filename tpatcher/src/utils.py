...
def main_args(parser):
    ...
    parser.add_argument('--example_repeat', type=int, default=1, help='we concatenate many examples in to one batch')
...
def get_score(model, batch, device, args=None, loclity=False):
    if args.model_type == 't5':
        ...
        correct_ids = batch["trg_input_ids"].to(device)

        assert correct_ids.size(1) == logits.size(1)
        ...
        score = torch.mean(
            (correct_ids[:, :-1] == ans[:, :-1]).float(),
            dim=-1
        ).detach().cpu().numpy().tolist()[0]

        return score

def get_score_gptj(model, batch, device, args=None, loclity=False):
    if args.model_type == 'gptj':

        correct_ids = batch["trg_input_ids"].to(device)
        target_length = correct_ids.size(1)
        logits = model.model(
        ...
        score = torch.mean(
            (correct_ids == ans).float(),
            dim=-1
        ).detach().cpu().numpy().tolist()[0]

        return score

def edit_or_not_seq2seq(editor, model, data_point, device, test_rephrases=True, args=None, before=False):

    with torch.no_grad():
        model.eval()
        editor.original_model.eval()
        if args.model_type == 't5':
            model.to(device)
            editor.original_model.to(device)
        else:
            if(len(args.gpus) == 2):
                device_map = {
        ...
        if args.model_type == 't5':
            score = get_score(model, batch, device, args=args)

            need_edit = 1 - score
            if test_rephrases:
                rephrase_inputs = {}

                rephrase_inputs['input_ids'] = batch["re_src_input_ids"].to(device)
                rephrase_inputs['attention_mask'] = batch["re_src_attention_mask"].to(device)

                rephrase_length = batch["re_src_input_ids"].size(0)
                if(batch["trg_input_ids"].size(0) != rephrase_length):
                    assert batch['trg_input_ids'].size(0) == 1
                    rephrase_length["decoder_input_ids"] = batch['trg_input_ids'].repeat(rephrase_length,1).to(device)
                    rephrase_length["decoder_attention_mask"] = batch['trg_attention_mask'].repeat(rephrase_length, 1).to(device)
                else:
                    rephrase_inputs['decoder_input_ids'] = batch["trg_input_ids"].to(device)
                    rephrase_inputs['decoder_attention_mask'] = batch["trg_attention_mask"].to(device)

                rephrase_logits = model.model(**rephrase_inputs).logits

                ans = torch.argmax(rephrase_logits, dim=-1)
                correct_ids = batch["trg_input_ids"].to(device)

                correct_count = 0
                for ans_ in ans:
                    correct_count += torch.mean(
                        (correct_ids[:,:-1] == ans_[None,:-1]).float(),
                        dim=-1
                    ).detach().cpu().numpy().tolist()[0]
                correct_count /= rephrase_length
            ...
            score = get_score_gptj(model, batch, device, args=args)
            ...
            if test_rephrases:
                ...
                correct_ids = batch["trg_input_ids"].to(device)
                target_length = correct_ids.size(1)
                rephrase_inputs['input_ids'] = batch["re_src_trg_input_ids"].to(device)
                rephrase_inputs['attention_mask'] = batch["re_src_trg_attention_mask"].to(device)
                rephrase_length = batch["re_src_trg_input_ids"].size(0)

                rephrase_logits = model.model(**rephrase_inputs).logits[:,-target_length:,:]
                ans = torch.argmax(rephrase_logits, dim=-1)

                correct_count = 0
                for ans_ in ans:
                    correct_count += torch.mean(
                        (correct_ids[:,:] == ans_[None,:]).float(),
                        dim=-1
                    ).detach().cpu().numpy().tolist()[0]
                correct_count /= rephrase_length
                re_num = rephrase_length
            ...
            if not before:
                ...
                if 'edit_unrelated_relation' in batch.keys():
                    pre_unrelated_relation_distribution = get_score_gptj(editor.original_model, batch['edit_unrelated_relation'], device, args=args, loclity=True)
                    post_unrelated_relation_distribution = get_score_gptj(model, batch['edit_unrelated_relation'], device, args=args, loclity=True)
                    unrelated_relation_score = torch.mean(
                        (pre_unrelated_relation_distribution == post_unrelated_relation_distribution).float(),
                        dim=-1
                    ).detach().cpu().numpy().tolist()[0]
                else:
                    unrelated_relation_score = None

                if 'edit_distracting' in batch.keys():
                    pre_distracting_distribution = get_score_gptj(editor.original_model, batch['edit_distracting'], device, args=args, loclity=True)
                    post_distracting_distribution = get_score_gptj(model, batch['edit_distracting'], device, args=args, loclity=True)
                    distracting_score = torch.mean(
                        (pre_distracting_distribution == post_distracting_distribution).float(),
                        dim=-1
                    ).detach().cpu().numpy().tolist()[0]
                else:
                    distracting_score = None

                if 'edit_portability' in batch.keys():
                    portability_score = get_score_gptj(model, batch['edit_portability'], device, args=args)
                else:
                    portability_score = None

                if 'edit_inverse' in batch.keys():
                    inverse_score = get_score_gptj(model, batch['edit_inverse'], device, args=args)
                else:
                    inverse_score = None

                if 'edit_tongyici' in batch.keys():
                    tongyici_score = get_score_gptj(model, batch['edit_tongyici'], device, args=args)
                else:
                    tongyici_score = None
...
def count_error_nums(model, data_point, device):
    ...
    #     if 'src_trg_input_ids' in batch.keys() and 'src_trg_attention_mask' in batch.keys():
    #         target_length = batch["trg_input_ids"][[0]].size(1)
    #         input_length = batch["src_trg_input_ids"][[0]].size(1) - target_length
    #         raw_logits = model(
    #             input_ids=batch["src_trg_input_ids"][[0]].to(device),
    #             attention_mask=batch["src_trg_attention_mask"][[0]].to(device),
    #         ).logits
    #         trg = batch["src_trg_input_ids"][[0], -target_length:].to(device)
    #         pred = torch.argmax(raw_logits, dim=-1)[:, -target_length:]
    #     else:
    #         input_length = 0
    #         raw_logits = model(
    #             input_ids=batch["src_input_ids"][[0]].to(device),
    ...
    #     temp_c = (pred != trg).cpu().squeeze()
    #
    #     import pdb
    #     pdb.set_trace()
    #     if temp_c.ndim == 0:
    #         select_index = [input_length + temp_c.item()]
    #     else:
    #         select_index = [input_length + i for i, s in enumerate(list(temp_c)) if s]
    #
    ...
    with torch.no_grad():
        ...
        if 'src_trg_input_ids' in batch.keys() and 'src_trg_attention_mask' in batch.keys():
            ...
            trg = batch["src_trg_input_ids"][[0], :].to(device)
        else:
            input_length = 0
            raw_logits = model(
                input_ids=batch["src_input_ids"][[0]].to(device),
            ...
            trg = batch["trg_input_ids"][[0], 1:].to(device)
        pred = torch.argmax(raw_logits, dim=-1)
        select_index = [i for i, s in enumerate(list((pred != trg).cpu().squeeze())) if (s and i >= input_length)]
