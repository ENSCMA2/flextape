...
def gen_metrics(p, result_dir, n, presult_dir, method, patch = False,):
	...
	for question in the_question:
		...
		for race in racial_groups:
			that_race = [i for i in range(len(race_available)) if match(races[i], race)]
			by_race[race] = that_race
		by_geo = {}
		for race in geo_groups:
			that_race = [i for i in range(len(geo_available)) if match(geos[i], race)]
			by_race[race] = that_race
		...
		if race_probs is not None:
			print(by_race)
			race_diffs = get_race_breakdown(by_race, race_probs)
			race_kls = get_multi_kl(by_race, race_probs)
		...
		if geo_probs is not None:
			geo_diffs = get_race_breakdown(by_geo, geo_probs)
			geo_kls = get_multi_kl(by_geo, geo_probs)
		...
		if race_pre_probs is not None:
			race_diffs_pre = get_race_breakdown(by_race, race_pre_probs)
			race_kls_pre = get_multi_kl(by_race, race_pre_probs)
		...
		if geo_pre_probs is not None:
			geo_diffs_pre = get_race_breakdown(by_geo, geo_pre_probs)
			geo_kls_pre = get_multi_kl(by_geo, geo_pre_probs)
			for k, v in geo_diffs_pre:
				try:
