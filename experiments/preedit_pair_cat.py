...
coc_list = set(countries_of_citizenship["Country"].tolist())
continents_lookup_p27 = {tem["Country"]: tem["Continent"] for i, tem in countries_of_citizenship.iterrows()}

CAND_DICT = {
    "P101": set([fow_flipped[f] if f in fow_flipped.keys() else "Miscellaneous" for f in fow]),
    "P27": set([continents_lookup_p27[c] for c in coc_list]),
    "P19": set([city_to_continent[c] if c in city_to_continent.keys() else "Other" for c in city_names])
}
def log(message):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ds_name",
        choices=["P21_P101", "P21_P19", "P27_P101", "P27_P19", "P101_P27", "P19_P101"],
        default="mcf",
